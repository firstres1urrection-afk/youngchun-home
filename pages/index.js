import Head from 'next/head';

export default function Home() {
  const handleCheckout = async (event) => {
    event.preventDefault();
    try {
      const response = await fetch('/api/create-checkout-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      const data = await response.json();
      if (data.url) {
        window.location.href = data.url;
      } else {
        alert('결제 오류: ' + data.error);
      }
    } catch (error) {
      alert('결제 처리 중 오류 발생');
    }
  };

  return (
    <>
      <Head>
        <title>영춘 - 자유를 향한 여정</title>
      </Head>
      <style jsx>{`
        body { font-family: sans-serif; margin: 0; padding: 0; line-height: 1.6; }
        .hero { background: linear-gradient(135deg, #0b3d91, #1e90ff); color: white; text-align: center; padding: 80px 20px; }
        .hero h1 { margin: 0; font-size: 48px; }
        .hero p { font-size: 20px; margin: 10px 0 20px; }
        .cta { background: white; color: #0b3d91; padding: 10px 20px; border-radius: 4px; text-decoration: none; font-weight: bold; }
        .section { padding: 40px 20px; max-width: 800px; margin: 0 auto; }
        h2 { margin-top: 0; }
      `}</style>
      <div className="hero">
        <h1>영춘</h1>
        <p>무한 자유와 부와 번영을 향한 여정</p>
      </div>
      <div className="section">
        <h2>About Youngchun</h2>
        <p>영춘은 현실적인 생각과 경제적 자유를 추구하는 사업자의 브랜드입니다. 무한한 자유와 풍요로운 번영을 꿈꾸며, 창립자인 병찬의 굴곡진 인생 여정을 통해 얻은 철학과 비전을 공유합니다. 우리의 여정은 도전과 회복의 연속이며, 자유로운 삶을 향한 열망을 담고 있습니다.</p>
      </div>
      <div className="section">
        <h2>Service Preview</h2>
        <p>우리는 ‘해외 부재 실시간 착신 서비스’를 비롯하여, 고객들이 언제 어디서나 연결될 수 있도록 돕는 다양한 기능을 준비하고 있습니다. 단순한 알림을 넘어 업무와 삶을 조율하는 통합 플랫폼을 목표로 하며, 향후에는 편리한 예약 시스템과 AI 기반 응대 서비스도 선보일 예정입니다.</p>
      </div>
      <div className="section">
        <h2>Contact &amp; Call to Action</h2>
        <p>궁금한 점이나 제안이 있으시면 언제든지 연락해 주세요. 현재 베타 이용권을 구매하실 수 있습니다. 아래 버튼을 통해 결제를 진행하시면, 해외 모드 설정 페이지로 이동합니다.</p>
        <button className="cta" onClick={handleCheckout}>베타 이용권 구매 (₩5,000)</button>
      </div>
    </>
  );
}
